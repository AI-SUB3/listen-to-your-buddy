...
目前 **v3.7**。版本號定義在建置腳本裡，同時寫進 `index.html` 的扉頁、`guide.html` 的頁首與更新紀錄，以及 service worker 的快取名稱。改版時三者一起更新，建置會檢查 `guide.html` 是否有對應的更新紀錄，沒有就中止。

完整的改動歷程見 [使用說明頁的更新紀錄](https://ai-sub3.github.io/listen-to-your-buddy/guide.html#更新紀錄)。

## 授權

© 2026 Aaron，保留所有權利。這份原始碼、設計與內容**未開放他人複製、散布、修改或另作他用**，包含非商業用途。你可以正常使用部署好的網站、可以連結分享，但請不要把原始碼另外拿去用在別的專案上。完整條款見 [LICENSE.md](LICENSE.md)。

底圖與 UBERON 本體論資料本身另有各自的授權（CC0、CC BY 3.0），不受上述限制，見上方「取自哪裡」與 `LICENSE.md` 裡的說明。
